...
#define SHADOWTEST defined(SHADERPASS_SHADOWCASTER)
	...
	// new - use the low res render target info
	...
	if (unity_OrthoParams.w < 0.5)
	{
		...
		// objPixelPos only needs to be known modulo pixel size.
		// See: https://github.com/ProPixelizer/ProPixelizer/issues/211
		objPixelPos = float2(
			mod(objPixelPos.x, pixelSize),
			mod(objPixelPos.y, pixelSize)
			);
	}
	...
	float2 macroPixelDelta = delta / max(pixelSize, 1);
	ditherUV = macroPixelDelta + float2(0.5, 0.5); 
